...
use crate::Page;
use crate::column::{BindFn, Bindable, Col, Cond, Order, bind_fn, quote_ident};
    ...
    T: Send + Unpin + for<'r> sqlx::FromRow<'r, SqliteRow>,
{
    ...
    pub async fn count(self, db: impl sqlx::SqliteExecutor<'_>) -> sqlx::Result<i64> {
        let mut qb = QueryBuilder::new(format!("SELECT COUNT(*) FROM {}", quote_ident(self.table)));
    ...
    pub async fn fetch_page(
        ...
        let mut count_qb =
            QueryBuilder::new(format!("SELECT COUNT(*) FROM {}", quote_ident(self.table)));
        push_where(&self.conds, &mut count_qb);
        let total = count_qb.build_query_scalar::<i64>().fetch_one(db).await?;

        // Saturating: `page` often comes straight from a query param, and
        // `i64::MAX` must mean "empty page far past the end", not an
        // overflow panic (debug) or a wrapped-negative offset (release).
        let mut qb = self.select_query(Some(per_page), Some((page - 1).saturating_mul(per_page)));
    ...
    fn select_query(
        ...
        let mut qb = QueryBuilder::new(format!("SELECT * FROM {}", quote_ident(self.table)));
        push_where(&self.conds, &mut qb);
...
impl UpdateBuilder {
    ...
    pub async fn execute(self, db: impl sqlx::SqliteExecutor<'_>) -> sqlx::Result<u64> {
        ...
        let mut qb = QueryBuilder::new(format!("UPDATE {} SET ", quote_ident(self.table)));
        for (i, (name, bind)) in self.sets.iter().enumerate() {
            if i > 0 {
                qb.push(", ");
            }
            qb.push(format!("{} = ", quote_ident(name)));
            bind(&mut qb);
        }
        push_where(&self.conds, &mut qb);
...
impl DeleteBuilder {
    ...
    pub async fn execute(self, db: impl sqlx::SqliteExecutor<'_>) -> sqlx::Result<u64> {
        let mut qb = QueryBuilder::new(format!("DELETE FROM {}", quote_ident(self.table)));
        push_where(&self.conds, &mut qb);
        let result = qb.build().execute(db).await?;
        Ok(result.rows_affected())
    }
